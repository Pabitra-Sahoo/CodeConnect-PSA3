...
const App = () => {
  ...
  const [code, setCode] = useState(() => {
    const savedState = localStorage.getItem("codeConnectSession");
    return savedState ? JSON.parse(savedState).savedCode : codeTemplates.javascript;
  });
  ...
  const handleLanguageChange = (e) => {
    ...
    if (window.confirm("Changing the language will remove the existing code. Do you want to continue?")) {
      setLanguage(newLanguage);
    ...
    return (
      <nav className={`navbar ${!isVisible ? "hidden" : ""}`}>
        ...
        <div className={`navbar-links ${isMenuOpen ? "active" : ""}`}>
          <a
            href="https://github.com/Pabitra-Sahoo/CodeConnect-PSA3"
